    ...
    var KEY     = { ESC: 27, SPACE: 32, LEFT: 37, UP: 38, RIGHT: 39, DOWN: 40, ENTER: 13, PAGE_DOWN: 34 },
    DIR     = { UP: 0, RIGHT: 1, DOWN: 2, LEFT: 3, MIN: 0, MAX: 3, AI: -1, SIM: -2},
    stats   = new Stats(),
    canvas  = get('canvas'),
    ...
    function keydown(ev) {
        ...
        if (playing) {
            switch(ev.keyCode) {
                ...
                case KEY.PAGE_DOWN: actions.push(DIR.SIM);    handled = true; break;
            }
        }
    ...
    function handle(action) {
        switch(action) {
            ...
            case DIR.SIM:   gradientDescent(0.003, 10, 0.05); break;
        }
    }
    ...
    var weights = {aggregateHeight: -0.51, completeLines: 0.76, holes: -0.36, bumpiness: -0.18}
    ...
    function runBenchmark() {
        ...
        return results.sort()[benchmarkSize / 2];
    }

    function calculateGradient(dx) {
        let currentScore = runBenchmark();
        console.log("Current score: ", currentScore)
        return Object.fromEntries(
            Object.entries(weights).map(([x, _]) => {
                weights[x] += dx;
                let newScore = runBenchmark();
                weights[x] -= dx;
                return [x, (newScore - currentScore) / (newScore * dx)];
            })
        );
    }

    function gradientDescent(learningRate, steps, dx) {
        for (let i = 0; i < steps; i++) {
            let gradient = calculateGradient(dx);
            weights = Object.fromEntries(Object.entries(weights).map(([x, y]) =>
                [x, y + gradient[x] * learningRate]
            ));
